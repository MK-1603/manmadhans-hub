...
export const MyRegistry = () => {
  ...
  const fetchTools = async () => {
    try {
      const cachedTools = localStorage.getItem('offline_my_registry_data');
      if (cachedTools) {
        try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:4000'}/api/v1/user-tools`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      if (res.ok) {
        const data = await res.json();
        setTools(data.tools || []);
        localStorage.setItem('offline_my_registry_data', JSON.stringify(data.tools || []));
      }
    } catch (err) {
      console.error('Failed to sync user registry', err);
      if (localStorage.getItem('offline_my_registry_data')) {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:4000'}/api/v1/user-tools/${id}`, {
        method: 'DELETE',
        headers: {
      ...
      <div className="flex-none flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 pb-5 border-b border-[var(--border2)] mb-6">
        <div className="min-w-0 space-y-1.5">
          ...
          <h1 className="font-royal text-2xl md:text-3xl font-black text-[var(--text)] tracking-tight flex items-center gap-3 leading-none">
            <Calendar className="w-6 h-6 text-[var(--neon)] shrink-0 animate-pulse" />
            Global Registry
          ...
          <p className="text-[12px] md:text-[13px] font-medium text-[var(--muted)] tracking-wide">
            {isAdministrator 
              ? "Dual telemetry chamber displaying system admin & user intelligence deployments."
              : "Cybernetic logging chamber for your deployed user intelligence assets."}
          </p>
        </div>
      </div>
